...
export default function AppleIcon() {
  ...
  return new ImageResponse(
    (
      <div style={{ display: "flex", flexDirection: "column", width: 180, height: 180, borderRadius: 45, overflow: "hidden", background: bg }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", background: dark, width: "100%", height: HDR }}>
          <div style={{ color: bg, fontSize: sc(21), fontWeight: 900, letterSpacing: 6, fontFamily: "sans-serif" }}>KOSPI</div>
        </div>
        ...
        <svg width="180" height={SVG_H} viewBox={`0 0 180 ${SVG_H}`} style={{ display: "block" }}>
          {([[20,16],[52,9],[92,14],[140,11],[158,32],[167,84],[163,118],[30,120],[13,64]] as number[][]).map(([x,y],i) => (
            <circle key={i} cx={x} cy={y} r="1.3" fill="#cccccc" opacity="0.5" />
          ))}
