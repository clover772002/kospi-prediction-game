  ...
  const handleUnlock = async () => {
    ...
    try {
      ...
      if (e instanceof InsightInsufficientTokensError) {
        setErr(
          `토큰이 부족해요 · 필요 ${e.detail.required ?? "?"}개 / 보유 ${e.detail.balance ?? "?"}개. 상점에서 충전해 주세요.`,
        );
      } else {
      ...
      {paywallLocked ? (
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap items-center gap-2 text-[11px]">
            <span className="text-amber-300 font-black tabular-nums">{data.price_tokens ?? 70} 토큰으로 열람</span>
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              disabled={unlocking}
              className="flex-1 min-w-[8rem] py-3 rounded-xl bg-teal-600 hover:bg-teal-500 disabled:opacity-50 text-white text-xs font-black transition-all active:scale-[0.98]"
            >
